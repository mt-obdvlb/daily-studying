...
void query(int rt,Node &p,int dep){
	...
	if(p.x[dim]>=kd[rt].x[dim])swap(lc,rc);
	if(sz[lc])query(lc,p,dep+1);
	if(res.first==-1){
		if(cur.second.x[2]<=p.x[2])res =cur;
		flag = 1;
	}else{
		if(cur.second.x[2]<=p.x[2]&&(cur.first<res.first
		||(cur.first==res.first&&cur.second.id<res.second.id)))res =cur;
		if(sq((ll)kd[rt].x[dim]-p.x[dim])<=res.first)flag=1;
	}
	if(sz[rc]&&flag)query(rc,p,dep+1);
}

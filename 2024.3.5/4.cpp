...
bool dfs(int** board,int i,int j,int dir,int n,int m){
	if(dir==0){
        ...
        if(i+i+d[dir][0]<1||i+d[dir][0]>n||j+d[dir][1]<1||j+d[dir][1]>m)return false;
...
int main(){
	cin>>n>>m;
	int** board = (int**)calloc(1005,sizeof(int*));
	board[0]=(int*)calloc(1005,sizeof(int));
	board[n+1]=(int*)calloc(1005,sizeof(int));
	for(int i = 1;i<=n;i++){
		board[i]=(int*)calloc(1005,sizeof(int));
		for(int j = 1;j<=m;j++){
			scanf("%d",&board[i][j]);
		}
	}
